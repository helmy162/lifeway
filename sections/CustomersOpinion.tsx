...
export default function CustomersOpinion() {
  return (
    <div className="w-full">
      ...
      {/* Image carousel */}
      <div className="mb-10 rounded-2xl bg-white px-5 pb-9 pt-12 text-black">
        <AliceCarousel
          autoPlay
          ...
          items={testimonials.map((testimonial) => (
            <div className="flex flex-col items-center gap-1">
              ...
              <div className="flex gap-0.5 text-star mb-2">
                {Array.from({ length: testimonial.rating }).map((_, i) => (
                  <StarIcon filled key={i} width={20} />
                ))}
                {Array.from({ length: 5 - testimonial.rating }).map((_, i) => (
                  <StarIcon filled={false} key={i} width={20} />
                ))}
              </div>
              <p className="text-center text-darkGray leading-5">
